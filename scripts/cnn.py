...
class IDCNN(nn.Module):
    ...
    def forward(self, embeddings):
        embeddings = self.linear(embeddings)  # [4,200,64]
        embeddings = embeddings.permute(0, 2, 1)  # [4,64,200]
...
class LayerNorm(nn.Module):
    ...
    def forward(self, x): # [4 64 200]
        mean = x.mean(-1, keepdim=True) # [4 64 1]
        # print(x.shape[-1])

        self.a_2 = nn.Parameter(torch.ones(x.shape[-1])).cuda()
        self.b_2 = nn.Parameter(torch.ones(x.shape[-1])).cuda()
